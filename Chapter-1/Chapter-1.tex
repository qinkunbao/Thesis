...
Side-channel attacks allow an attacker to infer sensitive information based on ``side-channel'' information. Examples of the ``side-channel'' information include execution timing, CPU usages, power consumption, and even sound. For example, in 1996, Kocher demonstrated that an attacker could break cryptosystems like RSA and Diffie-Hellman by measuring the execution time of a simple modular exponentiation operation. The simple modular exponentiation calculates the remainder of an exponent divided by an integer.  The simple modular exponentiation operation executes faster when the exponent bit is 0 and slower when the exponent bit is 1. An attacker can infer the whole secret key by measuring the execution time of the operation bit by bit.
