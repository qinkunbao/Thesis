...
Address-based side-channel attacks happen when a program has different behaviors when it accesses different memory addresses. In many situations,  computer systems allow the attacker to shares some hardware resources (e.g., CPU Cache, TLB, and DRAM) with the victim~\cite{ge2018survey,szefer2019survey}. As a result, an attacker can probe the shared hardware resource to infer the sensitive information.
While the root reason of the attack is the same, there are various approaches that can exploit the attack at different granularities. 
