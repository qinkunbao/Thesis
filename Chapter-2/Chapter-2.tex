...
In time-based attacks, an adversary can monitor the execution time of a victim. It could be the overall execution time or the execution time of some interesting points (e.g. a function).  The execution time is correlated to some events (e.g., cache hits, cache misses, page faults) during the execution. Example of those attacks include remote timing attacks, Bernstein attack on AES, and Kocher’s original attack from CRYPTO 96.
\subsubsection*{Access-based Attacks}
In access-based attacks, an adversary can 
