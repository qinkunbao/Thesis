...
In timing-based attacks, an adversary can monitor the execution time of a victim. It could be the overall execution time or the execution time of some interesting points (e.g. a function).  The execution time is correlated to some events (e.g., cache hits, cache misses, page faults, the number of instructions) during the execution. Examples of those attacks include remote timing attacks~\cite{brumley2005remote}, Bernstein's attack on AES~\cite{bernstein2005cache}, and Kocher’s original attack~\cite{kocher1996timing} from CRYPTO 96.
