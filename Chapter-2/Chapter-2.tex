...
\textbf{Flush+Reload.} Flush+Reload allows an attacker to observe the memory at the granularity of a cache line instead of a cache set. While Flush+Reload targets a single cache line, it requires the attacker and victim to share some memory. During the ``flush'' stage, the attacker flushes the ``monitored
